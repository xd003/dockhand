...
SET
	...
	`auto_update_schedule` = (
		...
		WHERE `git_stacks`.`repository_id` = `git_repositories`.`id`
			AND `git_stacks`.`auto_update` = 1
		ORDER BY
			CASE
				WHEN substr(COALESCE(`git_stacks`.`auto_update_cron`, ''), 1, 2) = '*/' THEN 6
				WHEN substr(COALESCE(`git_stacks`.`auto_update_cron`, ''), instr(COALESCE(`git_stacks`.`auto_update_cron`, ''), ' ') + 1, 1) = '*' THEN 5
				WHEN substr(COALESCE(`git_stacks`.`auto_update_cron`, ''), instr(COALESCE(`git_stacks`.`auto_update_cron`, ''), ' ') + 1, 2) = '*/' THEN 5
				WHEN `git_stacks`.`auto_update_schedule` = 'daily' THEN 3
				WHEN substr(COALESCE(`git_stacks`.`auto_update_cron`, ''), length(COALESCE(`git_stacks`.`auto_update_cron`, '')), 1) = '*' THEN 2
				ELSE 1
			END DESC,
			`git_stacks`.`id` ASC
		LIMIT 1
...
WHERE `id` IN (
	SELECT DISTINCT `repository_id` FROM `git_stacks` WHERE `auto_update` = 1
)
AND IFNULL(`auto_update`, 0) = 0;--> statement-breakpoint
...
UPDATE `git_stacks` SET `force_redeploy` = 1 WHERE `webhook_enabled` = 1;--> statement-breakpoint
...
SET
	...
	`webhook_secret` = (
		...
		WHERE `git_stacks`.`repository_id` = `git_repositories`.`id`
			AND `git_stacks`.`webhook_enabled` = 1
...
WHERE `id` IN (
	...
	WHERE `webhook_enabled` = 1 AND `webhook_secret` IS NOT NULL
)
AND IFNULL(`webhook_enabled`, 0) = 0;
