 ...
 */
export function validateComposePathsInput(
	value: unknown,
	options: { allowAbsolutePrimary?: boolean } = {}
): string | null {
	...
	for (const [index, p] of (value as string[]).entries()) {
		if (p.split(/[\\/]/).includes('..')) {
		...
		if (isAbsolute(p) && !(options.allowAbsolutePrimary && index === 0)) {
			return `Absolute paths not allowed in compose path "${p}"`;
		}
	}
	return null;
}
