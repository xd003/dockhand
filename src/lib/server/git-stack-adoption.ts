...
export async function validateExternalGitAdoption(
	...
	if (!source.composePath) return invalid('The external stack has no authoritative compose path', 409);

	const sourcePath = resolve(source.composePath);
	...
	if (!sameDirectory && existsSync(destinationDir)) return invalid(`Managed Git directory already exists: ${destinationDir}`, 409);
	return {
		ok: true,
		stackName,
		environmentId,
				...
				sourceCommit: async () => {
					try {
						await upsertStackSource({
							...
							composePaths: parseComposePathsColumn(gitStack.composePaths),
							envPath: targetEnvPath ?? (input.envFilePath ? null : preservedEnv.path ?? source.envPath ?? null)
						});
					} catch (error) {
