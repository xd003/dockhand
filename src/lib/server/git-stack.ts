...
import { existsSync, rmSync, readFileSync, realpathSync, mkdirSync, writeFileSync, renameSync, cpSync, readdirSync } from 'node:fs';
...
import { parseComposePathsColumn, firstComposePathOutsideDir } from './compose-files';
import {
	buildGitEnv,
	cleanupSshKey,
	buildRepoUrl,
...
const PENDING_CLONE_MARKER = '.dockhand-pending-clone.json';
...
export function cleanupPendingGitClones(maxAgeMs = 24 * 60 * 60 * 1000): void {
...
/** Move a pending checkout into its stack-owned location before first deploy. */
export async function adoptPendingGitClone(
	stackId: number,
	token: string
): Promise<{ success: boolean; path?: string; error?: string }> {
	const gitStack = await getGitStack(stackId);
	const record = readPendingClone(token);
	if (!gitStack || !record || record.repositoryId !== gitStack.repositoryId) {
		return { success: false, error: 'Pending repository checkout is invalid or expired' };
	}

	const destination = await getStackRepoPath(stackId, gitStack.stackName, gitStack.environmentId);
	if (existsSync(destination)) {
		return { success: false, error: 'The stack repository directory already exists' };
	}

	try {
		mkdirSync(dirname(destination), { recursive: true });
		try {
			renameSync(pendingClonePath(token), destination);
		} catch (error: any) {
			if (error?.code !== 'EXDEV') throw error;
			cpSync(pendingClonePath(token), destination, { recursive: true, force: false });
			rmSync(pendingClonePath(token), { recursive: true, force: true });
		}
		writeFileSync(join(destination, PENDING_CLONE_MARKER), JSON.stringify({ repositoryId: record.repositoryId, branch: record.branch }));
		rmSync(pendingCloneMetadataPath(token), { force: true });
...
function isAdoptedPendingClone(repoPath: string, repositoryId: number): boolean {
	try {
		const marker = JSON.parse(readFileSync(join(repoPath, PENDING_CLONE_MARKER), 'utf-8')) as { repositoryId?: number };
		return marker.repositoryId === repositoryId;
	} catch {
		return false;
	}
}

function consumeAdoptedPendingClone(repoPath: string): void {
	try { rmSync(join(repoPath, PENDING_CLONE_MARKER), { force: true }); } catch { /* best effort */ }
}
...
export async function syncGitStack(stackId: number, _onProgress?: ProgressCallback): Promise<SyncResult> {
	...
	try {
		...
		// A newly-created stack may already own a checkout moved from the create-flow
		// pending directory. Consume it once instead of cloning the repository again.
		const adoptedPendingClone = isAdoptedPendingClone(repoPath, stackId === gitStack.id ? gitStack.repositoryId : -1);
		// Always re-clone normal stack syncs to ensure clean state (handles branch/URL/
		// credential changes and force pushes). Blobless clones fetch commits but load
		// blobs on demand. Fall back to DB lastCommit when the repo was deleted (#693).
		const previousCommit = adoptedPendingClone ? null : await getPreviousCommit(repoPath, env) ?? gitStack.lastCommit ?? null;
		if (existsSync(repoPath) && !adoptedPendingClone) {
			console.log(`${logPrefix} Removing existing clone for fresh sync...`);
			rmSync(repoPath, { recursive: true, force: true });
		}

		if (adoptedPendingClone) {
			console.log(`${logPrefix} Using repository checkout moved from the create flow`);
		} else {
		...
		// Every configured compose file must reside within the copied source
		// directory (contextDir or the primary file's dir) — change detection,
		// the directory copy, and path rebasing all operate on that directory
		// only, so a file outside it can never sync or deploy correctly.
		const configuredPaths = parseComposePathsColumn(gitStack.composePaths);
		const relBaseDir = gitStack.contextDir || dirname(gitStack.composePath);
		const outsidePath = firstComposePathOutsideDir(configuredPaths, relBaseDir);
		if (outsidePath) {
			throw new Error(
				`Configured compose file "${outsidePath}" is outside the source directory "${relBaseDir}". ` +
				`Every compose file must reside within it (adjust contextDir or the file set).`
			);
		}
...
async function deployGitStackWithProgressCore(
	...
	try {
		...
		const adoptedPendingClone = isAdoptedPendingClone(repoPath, gitStack.repositoryId);
		// Fall back to DB lastCommit when repo dir was deleted by a previous failed sync (#693).
		const previousCommit = adoptedPendingClone ? null : await getPreviousCommit(repoPath, env) ?? gitStack.lastCommit ?? null;

		// Step 2: Cloning
		onProgress({ status: 'cloning', message: adoptedPendingClone ? 'Using pre-cloned repository...' : 'Cloning repository...', step: 2, totalSteps });
		...
		if (adoptedPendingClone) {
			onProgress({ status: 'fetching', message: 'Using the selected repository checkout', step: 3, totalSteps });
		} else {
			assertSafeGitRef(repo.branch);
			const repoUrl = buildRepoUrl(repo.url, credential);
