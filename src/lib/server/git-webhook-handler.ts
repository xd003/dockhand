...
import { allowSecretlessWebhook, decideWebhookSecretPolicy } from './webhook-secret-policy';
...
async function finishWebhookTrigger<TEntity extends GitWebhookEntity>(
	...
	method: 'GET' | 'POST',
	source: string,
	verification?: 'skipped_no_secret'
): Promise<Response> {
...
export async function handleGitWebhookRequest<TEntity extends GitWebhookEntity>(
	...
	if (request.method === 'GET') {
		const policy = decideWebhookSecretPolicy(!!entity.webhookSecret, allowSecretlessWebhook());
		...
		if (policy.action === 'deploy-unverified') {
			// ALLOW_WEBHOOKS_WITHOUT_SECRET opt-in (isolated network): trigger without
			// verification, but record the unverified trigger in the audit trail.
			const result = await options.trigger(id);
			return finishWebhookTrigger(event, id, entity, options, result, 'GET', 'get', 'skipped_no_secret');
		}
	...
	if (policy.action === 'deploy-unverified') {
		...
		// verification, but record the unverified trigger in the audit trail.
		const result = await options.trigger(id);
		return finishWebhookTrigger(event, id, entity, options, result, 'POST', source, 'skipped_no_secret');
	}
