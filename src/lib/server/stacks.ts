...
async function readDirFilesAsMap(dirPath: string): Promise<{ files: Record<string, string>; modifiedTimes: Record<string, number>; skipped: string[] }> {
	...
	async function scanDir(currentPath: string, relativePath: string = ''): Promise<void> {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				modifiedTimes[relPath] = Math.trunc(stat.mtimeMs);
			}
		}
	}
...
async function hawserSupportsFileMtimeSync(envId: number): Promise<boolean> {
	const { getEdgeAgentCapabilities } = await import('./hawser.js');
	const edgeCaps = getEdgeAgentCapabilities(envId);
	if (edgeCaps.length > 0) return edgeCaps.includes('file-mtime-sync');
	try {
		const { dockerFetch } = await import('./docker.js');
		const res = await dockerFetch('/_hawser/info', { method: 'GET' }, envId);
		if (!res.ok) return false;
		const info: any = await res.json();
		return Array.isArray(info?.capabilities) && info.capabilities.includes('file-mtime-sync');
	} catch {
		return false;
	}
}
...
async function executeComposeViaHawser(
	...
	try {
		...
		if (stackFileModifiedTimes && Object.keys(stackFileModifiedTimes).length > 0 && !(await hawserSupportsFileMtimeSync(envId))) {
			return {
				success: false,
				error: `Stack "${stackName}" uses newest-file-wins synchronization, but the Hawser agent does not support it. Update the Hawser agent (file-mtime-sync capability) and reconnect.`
			};
		}
