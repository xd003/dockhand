...
function readEffectiveComposeFileContents(absolutePaths: string[], explicitPaths = false): {
	...
	for (const f of effectiveFiles) {
		...
		try {
			composeContents[f.path] = readFileSync(f.path, 'utf-8');
		} catch {
...
export async function saveStackComposeFile(
	...
	if (
		...
		const existingPaths = source?.composePaths ? parseComposePathsColumn(source.composePaths) : [];
		await upsertStackSource({
			stackName: name,
			environmentId: envId ?? null,
			sourceType: 'internal',
			composePath: options?.composePath || source?.composePath || null,
			composePaths: options?.composePaths ?? (existingPaths.length ? existingPaths : null),
