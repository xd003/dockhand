...
import { linkedFileLanguage, MAX_LINKED_FILE_SIZE } from '$lib/stack-linked-files';
...
function readDraftFile(root: string, path: string): string {
	...
	if (bytes.byteLength > MAX_LINKED_FILE_SIZE) throw new Error(`Draft file exceeds the ${MAX_LINKED_FILE_SIZE} byte limit`);
	let content: string;
	try {
...
function linkedFileRevision(content: string): string {
	...
	try {
		...
		const entries = paths.map((path) => {
			...
			return {
				...
				revision: linkedFileRevision(content)
			};
		});
