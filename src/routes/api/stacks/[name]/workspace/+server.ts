...
import { isBinaryWorkspaceContent, listWorkspace, MAX_STACK_WORKSPACE_TEXT_SIZE, normalizeWorkspacePath, resolveWorkspacePath, withInternalStackDirectory, workspaceRevision } from '$lib/server/stack-workspace';
...
async function context(name: string, url: URL, cookies: Parameters<typeof authorize>[0], permission: 'view' | 'edit') {
	...
	return { root: stack.stackDir, source };
}
...
async function gitDetails(root: string, source: Awaited<ReturnType<typeof getStackSource>>, path: string) {
	...
	const repoPath = relative(repoRoot, await resolveWorkspacePath(root, path)).split('/').join('/');
...
export const GET: RequestHandler = async ({ params, url, cookies }) => {
	try {
		const { root, source } = await context(params.name, url, cookies, 'view');
		...
		if (url.searchParams.get('content') !== '1') {
			...
			return json({ entries: await Promise.all(entries.map(async (entry) => ({
				...entry,
				git: await gitDetails(root, source, entry.path)
			}))) });
		}
		...
		const git = await gitDetails(root, source, path);
		return json({ binary: false, content: new TextDecoder().decode(content), size: content.byteLength, revision: workspaceRevision(content), git: git ? { tracked: git.tracked, ignored: git.ignored } : null });
	} catch (error) { return failure(error); }
};

export const POST: RequestHandler = async ({ params, url, cookies, request }) => {
	try {
		let { root, source: stackSource } = await context(params.name, url, cookies, 'edit');
...
export const PUT: RequestHandler = async ({ params, url, cookies, request }) => {
	try {
		const { root, source: stackSource } = await context(params.name, url, cookies, 'edit');
		const body = await request.json();
		if (typeof body.workspaceEnabled === 'boolean') {
			const rawEnv = url.searchParams.get('env');
			if (!await updateStackSource(params.name, rawEnv ? Number.parseInt(rawEnv) : null, { workspaceEnabled: body.workspaceEnabled })) throw Object.assign(new Error('Stack source not found'), { status: 404 });
		...
		let source = await resolveWorkspacePath(root, body.path);
		if (typeof body.content === 'string') {
			...
			if (typeof body.expectedRevision === 'string' && workspaceRevision(await readFile(source)) !== body.expectedRevision) throw Object.assign(new Error('File changed since it was opened; reload it and try again'), { status: 409 });
...
export const DELETE: RequestHandler = async ({ params, url, cookies, request }) => {
	try {
		const { root } = await context(params.name, url, cookies, 'edit');
		const { path } = await request.json();
		await rm(await resolveWorkspacePath(root, path), { recursive: true, force: false });
		return json({ success: true });
	} catch (error) { return failure(error); }
};
