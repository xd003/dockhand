...
<script lang="ts">
	...
	const ALL_TABS = [
		...
		{ value: 'config-sets', label: 'Config sets' },
	...
	const TABS = $derived(ALL_TABS.filter((tab) => tab.value !== 'backups' || $page.data.backupsEnabled));
	const activeTabMeta = $derived(TABS.find((t) => t.value === activeTab));
</script>

{#snippet tabIcon(value: (typeof ALL_TABS)[number]['value'], className: string)}
	{#if value === 'general'}<Settings class={className} />
	{:else if value === 'environments'}<Globe class={className} />
	{:else if value === 'labels'}<Tags class={className} />
	{:else if value === 'registries'}<Download class={className} />
	{:else if value === 'git'}<GitBranch class={className} />
	{:else if value === 'secrets'}<KeyRound class={className} />
	{:else if value === 'config-sets'}<Layers class={className} />
	{:else if value === 'notifications'}<Bell class={className} />
	{:else if value === 'backups'}<Archive class={className} />
	{:else if value === 'auth'}<Users class={className} />
	{:else if value === 'license'}<Crown class={className} />
	{:else}<Info class={className} />
	{/if}
{/snippet}
			...
			<Select.Root type="single" value={activeTab} onValueChange={(v) => { if (v) handleTabChange(v); }}>
				<Select.Trigger class="max-md:h-11! w-full" aria-label="Settings section">
					<div class="flex items-center gap-2 min-w-0">
						{#if activeTabMeta}
							{@render tabIcon(activeTabMeta.value, 'w-4 h-4 shrink-0 text-muted-foreground')}
					...
					{#each TABS as t (t.value)}
						<Select.Item value={t.value}>
							<div class="flex items-center gap-2.5">
								{@render tabIcon(t.value, 'w-4 h-4 text-muted-foreground')}
								<span>{t.label}</span>
							</div>
						</Select.Item>
					{/each}
		...
		<Tabs.List bind:ref={tabList} class="hidden w-full h-auto gap-1 p-1 flex-wrap justify-start md:flex">
			{#each TABS as t (t.value)}
				<Tabs.Trigger value={t.value} class="max-sm:shrink-0 max-sm:min-w-max sm:flex-1 flex items-center justify-center gap-1.5">
					{@render tabIcon(t.value, 'w-4 h-4')}
					{t.label}
				</Tabs.Trigger>
			{/each}
		</Tabs.List>
