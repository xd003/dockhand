...
import { detectedComposeOverridePaths } from '../src/lib/compose-overrides';

const stackDir = '/stack';

describe('detectedComposeOverridePaths', () => {
	it('returns every standard override in UI order next to the primary', () => {
		assert.deepEqual(
			detectedComposeOverridePaths('/stack/app/custom.yml', [
			...
			[
				'/stack/app/compose.override.yml',
				'/stack/app/compose.override.yaml',
				'/stack/app/docker-compose.override.yml',
				'/stack/app/docker-compose.override.yaml'
			]
		);
	});
});
...
describe('resolveEffectiveComposeFiles', () => {
	...
	it('does not re-add a removed override to an explicit single-file list', () => {
		const primary = join(stackDir, 'compose.yaml');
	...
	it('adds every standard override that exists', () => {
		const primary = join(stackDir, 'compose.yaml');
		...
		assert.deepEqual(composeFilePathList(files), [primary, ymlOverride, yamlOverride]);
	});
...
describe('validateComposePathsInput', () => {
	it('accepts undefined, null, and arrays of non-empty strings', () => {
		...
		assert.equal(
			validateComposePathsInput(['/stacks/1/myapp/compose.yaml'], { allowAbsolutePrimary: true }),
			null
		);
	});
	...
	it('rejects traversal and non-stack filenames (arbitrary file read guard)', () => {
		assert.match(validateComposePathsInput(['/etc/x.env'])!, /Absolute paths not allowed/);
		assert.match(validateComposePathsInput(['/abs/compose.yaml'])!, /Absolute paths not allowed/);
	...
	it('only permits an absolute primary for adopted stacks', () => {
		assert.equal(
			validateComposePathsInput(['/stacks/app/compose.yaml', 'override.yaml'], { allowAbsolutePrimary: true }),
			null
		);
		assert.match(
			validateComposePathsInput(['/stacks/app/compose.yaml', '/root/secret.env'], { allowAbsolutePrimary: true })!,
