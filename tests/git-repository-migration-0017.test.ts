...
const SQLITE_MIGRATION = resolve(
	new URL('../drizzle/0017_compose_paths.sql', import.meta.url).pathname
);
const PG_MIGRATION = resolve(
	new URL('../drizzle-pg/0017_compose_paths.sql', import.meta.url).pathname
);
...
describe('migration 0017 (compose_paths)', () => {
...
describe('migration 0017 file guards', () => {
	for (const [name, path] of [
		...
		it(`(${name}) is additive and does not drop columns`, () => {
			...
			assert.ok(!/UPDATE\s+["`]?git_/.test(sql), '0017 must not mutate git data');
		});
	}
});
