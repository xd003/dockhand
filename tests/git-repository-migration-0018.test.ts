...
const SQLITE_MIGRATION = resolve(
	new URL('../drizzle/0018_centralized_git.sql', import.meta.url).pathname
);
const PG_MIGRATION = resolve(
	new URL('../drizzle-pg/0018_centralized_git.sql', import.meta.url).pathname
);
...
describe('migration 0018 (centralized git engine)', () => {
...
describe('migration 0018 file guards', () => {
	for (const [name, path] of [
		...
		it(`(${name}) does not promote data — the stack→repo promotion moved to the runtime backfill`, () => {
			const sql = readFileSync(path, 'utf8');
			assert.ok(!/UPDATE\s+["`]?git_repositories/.test(sql), '0018 must not mutate git_repositories data (moved to runtime backfill)');
			assert.ok(!/force_redeploy\s*=\s*1/.test(sql), '0018 must not backfill force_redeploy (moved to runtime backfill)');
		});

		it(`(${name}) does not backfill engine in SQL — that is a runtime step`, () => {
			const sql = readFileSync(path, 'utf8');
			assert.ok(!/UPDATE\s+["`]?git_stacks/.test(sql), '0018 must not mutate git_stacks.engine in SQL (backfill is runtime)');
			assert.ok(!/SELECT.*settings/.test(sql), '0018 must not read the settings table in SQL');
		});
	}
});
